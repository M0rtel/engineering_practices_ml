...
**Скриншот:** Результат установки зависимостей через UV
*(Здесь должен быть скриншот вывода команды uv sync)*
...
UV автоматически создает и управляет виртуальным окружением. Виртуальное окружение создается при выполнении `uv sync`. Команды выполняются через `uv run`.
